...
from app.services.mean_reversion_strategies.mean_reversion_defaults import DEFAULT_LOOKBACK_DAYS, DEFAULT_FEE_RATE


def trades_to_metrics(trades: list[TradeResultDict], initial_capital: int) -> tuple[float, float, float, int]:
    if not trades:
