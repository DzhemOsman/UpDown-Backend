...
def optimize_money_management_with_grid_search(
    ...
    """
    Führt eine Grid-Search, alle Parameterkombinationen, zur Optimierung der
    Mean-Reversion-Strategie über die angegebenen Ticker und Parameterbereiche durch.

    :param is_trend: Bool, ob Mean-Reversion mithilfe des SMA berechnet werden soll.
    :param is_kadane: Bool, ob der Kadane-Algorithmus für die
    Signalgenerierung verwendet werden soll.
    :param allocation_options: Liste von Allokationsprozentsätzen
    (Anteil des verfügbaren Kapitals pro Trade).
    :param max_positions_options: Liste von maximalen gleichzeitigen
    Positionen im Portfolio.
    :param stop_loss_options: Liste von Stop-Loss-Schwellenwerten in Prozent
    (maximaler Verlust pro Position).
    :param tickers: Liste von Ticker-Symbolen, z.B., ['AAPL', 'MSFT'],
    die getestet werden.
    :param drop_options: Liste von Schwellenwerten (in Prozent)
    für den prozentualen Rückgang, der ein Kaufsignal auslöst.
    :param hold_options: Liste von Halteperioden in Tagen
    (Anzahl Tage, die eine Position maximal gehalten wird).
    ...
    """
    # itertools.product verhindert 6-fache Schleifen-Verschachtelung
    # (Arrow Anti-Pattern)
    ...
    return _find_best_combination(
        combinations, tickers, initial_capital, start, end, is_kadane, is_trend
    )
...
def optimize_money_management_with_randomized_grid_search(
    ...
    """
    Führt eine Grid-Search, über n_trials zufällige Parameterkombinationen,
    zur Optimierung der Mean-Reversion-Strategie über die angegebenen
    Ticker und Parameterbereiche durch.
    ...
    :param tickers: Liste von Ticker-Symbolen, z.B., ['AAPL', 'MSFT'],
    die getestet werden.
    :param drop_options: Liste von Schwellenwerten (in Prozent) für den
    prozentualen Rückgang, der ein Kaufsignal auslöst.
    :param hold_options: Liste von Halteperioden in Tagen (Anzahl Tage,
    die eine Position maximal gehalten wird).
    :param take_profit_options: Liste von Take-Profit-Zielen in Prozent
    (Gewinnziel zum Verkauf).
    :param stop_loss_options: Liste von Stop-Loss-Schwellenwerten in Prozent
    (maximaler Verlust pro Position).
    ...
    :param start: Startdatum des Backtests
    :param end: Enddatum des Backtests
    :param is_kadane: Bool, ob der Kadane-Algorithmus für die Signalgenerierung
    ...
    return _find_best_combination(
        sampled, tickers, initial_capital, start, end, is_kadane, is_trend
    )
...
def _find_best_combination(
    ...
    with ProcessPoolExecutor(
        ...
        for combo, roi, profit, win_rate, n_trades in executor.map(
            _evaluate, combinations, chunksize=10
        ):
            if roi > best_roi:
                best_roi = roi
                best_combo = combo
    ...
    bot.set_ticker_cache(
        ticker_data
    )  # vorgeladene Daten injizieren -> kein erneuter DB-Zugriff
